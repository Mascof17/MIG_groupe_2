...
P= 300*100000 #pression initialement à 300 bars
...
masse_volumique_out = [cp.PropsSI('Dmass' ,'T', T,'P', P_out[0]*100000 , 'H2')]
...
while P_out[-1]<500 : #on s'arrete à MP a 500bars
    ...
    if debit_massique > debit_max:
        debit_max=debit_massique
    ...
    P_out.append((cp.PropsSI('P', 'T', T, 'Dmass', masse_volumique_out[-1], 'H2'))/100000)
    Pression.append(P/100000)
    Temps.append(Temps[-1]+dt)
print(Temps[-1])
print((X[0]-X[-1])*V)
print(debit_max) #debit max en kg/s
print(debit_max*3600) #debit max en kg/h
